...
def build_ds():
    ...
    for idx in tqdm(range(len(train_df))):
        if idx > len(train_df) - 1 - SEQUENCE_LENGTH:
            ...
            train_inputs.append(
                tf.expand_dims(tf.convert_to_tensor(train_df.iloc[idx: idx + 15], dtype=tf.float32), axis=0))
            train_target = [1, 0, 0] if train_df.iloc[idx + 15, 7] < -STATUS_INDICATOR else [0, 1, 0] if train_df.iloc[
                                                                                                             idx + 15, 7] < STATUS_INDICATOR else [
                0, 0, 1]
